...
<button class="btn button_btn btn-danger" type="button" onclick="" id="delAll"><i class="fa fa-trash-o"></i>&nbsp;删除</button>
<span class="submenu"><a href="../index.php?r=products/add_product" name="add_product.html" class="btn button_btn bg-deep-blue" title="添加产品"><i class="fa  fa-edit"></i>&nbsp;添加产品</a></span>
<div class="search  clearfix">
 <label class="label_name">商品搜索：</label><input name="search" type="text"  class="form-control col-xs-6"/><button class="btn button_btn bg-deep-blue " onclick=""  type="button" id="search"><i class="fa  fa-search"></i>&nbsp;搜索</button>

</div>
<script>
	$("#search").click(function(){
		var name = $("input[name='search']").val();
		window.location.href='../index.php?r=products/products&name='+name;
	})
</script>
</div>
<!--列表展示-->
<div class="list_Exhibition margin-sx">
 <table class="table table_list table_striped table-bordered" id="sample-table">
  ...
  <tbody id="tbody">
   ...
   <tr role="row" class="odd"><td><label>
   	<input type="checkbox" class="ace"><span class="lbl"></span></label></td><td class="sorting_1"><?=$val['goods_id']?></td><td><?=$val['goods_name']?></td><td>55</td><td>234.50</td><td><?=$val['cate']?></td><td>4567</td><td>2016-03-12 12:23:12</td><td>
   	<span class="label label-success label-sm">上架</span>
   	</td><td> 
   	<a href="javascript:ovid()"   class="xiajia btn bg-green operation_btn">下架</a> 
   	<a href="javascript:ovid()" onclick="picture_edit(this," 123")"="" class="btn bg-deep-blue operation_btn">修改</a> <a href="javascript:ovid()"  class="btn btn-danger operation_btn del">删除</a> <a href="javascript:ovid()" onclick="picture_img(this,121)" class="btn bg-deep-blue operation_btn">图片</a></td></tr>
   <?php }?>
  </tbody>
 </table>
</div>
...
<script>
	$("#delAll").click(function(){
		var del = $("#tbody").find("tr");
		var str = '';
		$.each(del,function(){
			if($(this).find("input[type='checkbox']").is(":checked")){
				var id = $(this).find(".sorting_1").html();
				str+=id+'_';
			}
		})
		if(str == ''){
			...
			window.location.href='../index.php?r=products/delall&id='+str;
		}
		
		
	})
	$(".xiajia").click(function(){
		$(this).removeClass('xiajia');
		$(this).removeClass('bg-green');
		$(this).addClass('bg-black');
		$(this).parent().prev().find("span").html("下架");
	})
	$(".del").click(function(){
		var id = $(this).parents(".odd").find("td").eq(1).html();
		window.location.href='../index.php?r=products/del&id='+id;
		// $.ajax({
	 //      type: 'POST',
	 //      url: '../index.php?r=products/del',
	 //      data: {
	 //        'id':id
	 //      },
